...
public class PatronDAO {
    ...
    // default constructor
    public PatronDAO() {
        patronFile = new File("data/patron.csv");
        load();
        /*
    ...
    private void load() {
        try (Scanner sc = new Scanner(patronFile)) {
            while (sc.hasNext()) {
                String line = sc.nextLine();
                if (line == null || line.isEmpty()) break;
                Scanner lineSc = new Scanner(line);
                lineSc.useDelimiter(",");
                patronList.add(new Patron(lineSc.nextInt(), lineSc.next(), lineSc.next()));
            }
        } catch (IOException e) {
            e.printStackTrace();  //can be used for debugging
            throw new DataException("Can't read Patron data", e);
        }
    }
